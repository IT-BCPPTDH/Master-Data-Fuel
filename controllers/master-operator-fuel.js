...
const { insert, edit } = require('../query-service/operator_fuel/operator_service')

async function getFuelOperator() {
    try{
        const data = await db.query(QUERY_STRING.GET_FUEL_OPERATOR)
...
async function getFuelOperatorId(operatorId){
    try{
        const data = await db.query(QUERY_STRING.GET_DETAIL_OPERATOR_FUEL, [operatorId])
...
async function deleteFuelOperator(operatorId){
    try{
        const data = await db.query(QUERY_STRING.DELETE_OPERATOR, [operatorId])
        if(data){
            return {
              status: HTTP_STATUS.OK,
